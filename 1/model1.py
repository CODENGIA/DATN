...
def info_nce_loss(out_n, out_t, temperature=0.1, num_negatives=5, return_individual_loss=False):
    """
    Hàm InfoNCE Loss cho Contrastive Learning.
    Nếu return_individual_loss=True, trả về mảng Loss riêng biệt của từng gen (để tính độ kháng cự).
    """
    ...
    if return_individual_loss:
        # Không tính trung bình, trả về mảng chứa điểm kháng cự
        loss = F.cross_entropy(logits, labels, reduction='none')
    else:
        # Tính trung bình để tối ưu hóa trọng số lúc Train
